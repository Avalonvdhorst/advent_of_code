...
class Intcode
  ...
  def get_two_params
    @param1 = read_param(@code[@pos + 1], @modes[0])
    @param2 = read_param(@code[@pos + 2], @modes[1])
  end

  def write_pos(number, mode = 0)
    ret = 0
    case mode
    when 0 #position
      ret =  number
    when 1 #immediate
      ret =  number
    when 2 #relative
      ret = @relative_base + number
    end
    ret
  end

  def read_param(number, mode = 0)
    ret = 0
    case mode
    when 0 #position
      ret =  @code[number]
  ...
  def one
    ...
    get_two_params
    @code[write_pos(@code[@pos + 3], @modes[2])] = @param1 + @param2
    @pos += 4
  end
  ...
  def three
    ...
    @code[write_pos(@code[@pos + 1], @modes[0])] = input
    @pos += 2
  end

  def four
    output = read_param(@code[@pos + 1], @modes[0])
  ...
  def seven
    ...
    @code[write_pos(@code[@pos + 3], @modes[2])] = value
    @pos += 4
  end
  ...
  def nine
    @relative_base = read_param(@code[@pos + 1], @modes[0])
    p @relative_base
    @pos += 2
  end
